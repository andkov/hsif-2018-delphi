...
#   output = "./analysis/report-1/report-1.md"
# )
...
) 
{
  ...
  midpoint_observed <- ds_mid_points %>%
    dplyr::filter(label == "bar(italic(X))") %>% 
    dplyr::select(value) %>% 
    as.list() %>% unlist() %>% as.numeric()
  midpoint_cutoff <- 3.5
  
  fill_custom <- ifelse(midpoint_observed > midpoint_cutoff, "salmon","grey90")
  ...
  g <- ggplot2::ggplot(d_observed, ggplot2::aes_string(x = variable_name)) + 
    ...
    ggplot2::geom_vline(
      xintercept = ds_mid_points$line_position
      ,color = palette_midpoint_line 
      ,size = 1
      , na.rm = T
      # ,linetype  = "dashed"
    ) +
    ...
    ggplot2::coord_cartesian(ylim = c(0,23))
    ggplot2::labs(
...
matrix_plot <- function(
  ...
  d %>% dplyr::glimpse()
facet_y_values <- d %>% 
  dplyr::group_by_(.dots = facet_y) %>% 
  dplyr::summarize(mean = mean(response_value)) %>% 
  dplyr::arrange(desc(mean)) %>% 
  dplyr::select_(.dots = facet_y) %>% 
